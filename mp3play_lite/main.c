...
// info
PSP_MODULE_INFO("mp3play_lite", 0x1000, 0, 21);
...
u32 fg = RGB(255, 255, 255);
u32 bg = RGB(0, 0, 0);
u32 rg = RGB(255, 0, 0);

extern CANVAS canvas;		// DrawBuffer
...
int sceDmacplusLcdcSetBaseAddrPatched(void *baseaddr)
{
	...
	{
		DrawSmallFontPrintf(3, 200,					  fg, bg, "FLAG    = %s", flag > 0 ? fmsg[1] : fmsg[0]);
		DrawSmallFontPrintf(3, 208,					  fg, bg, "HOLD    = %s", hold == 0 ? "OFF" : "ON");
		DrawSmallFontPrintf(3, 216, (pos == 6) ? rg : fg, bg, "CLOCK   = %d/%d", cpulist[unk], buslist[unk]);
		DrawSmallFontPrintf(3, 224, (pos == 5) ? rg : fg, bg, "DRAW    = %s", smsg[draw_flag]);
		DrawSmallFontPrintf(3, 232, (pos == 4) ? rg : fg, bg, "SKIP    = %s", skip == 0 ? "FILE" : "FOLDER");
		DrawSmallFontPrintf(3, 240, (pos == 3) ? rg : fg, bg, "MUTE    = %s", hmsg[hook_run]);
		DrawSmallFontPrintf(3, 248, (pos == 2) ? rg : fg, bg, "MODE    = %s", mmsg[mode]);
		DrawSmallFontPrintf(3, 256, (pos == 1) ? rg : fg, bg, "VOLUME  = %d", (int)(volume / MINI_VOLUME));
		DrawSmallFontPrintf(3, 264, (pos == 0) ? rg : fg, bg, "MP3NAME = %s", list_name);
	}

	else if(drawing && time <= sceKernelLibcClock())
	{
		DrawSmallFontPrintf(3, 200,	bg, bg, "FLAG    = %s", flag > 0 ? fmsg[1] : fmsg[0]);
		DrawSmallFontPrintf(3, 208,	bg, bg, "HOLD    = %s", hold == 0 ? "OFF" : "ON");
		DrawSmallFontPrintf(3, 216, bg, bg, "CLOCK   = %d/%d", cpulist[unk], buslist[unk]);
		DrawSmallFontPrintf(3, 224, bg, bg, "DRAW    = %s", smsg[draw_flag]);
		DrawSmallFontPrintf(3, 232, bg, bg, "SKIP    = %s", skip == 0 ? "FILE" : "FOLDER");
		DrawSmallFontPrintf(3, 240, bg, bg, "MUTE    = %s", hmsg[hook_run]);
		DrawSmallFontPrintf(3, 248, bg, bg, "MODE    = %s", mmsg[mode]);
		DrawSmallFontPrintf(3, 256, bg, bg, "VOLUME  = %d", (int)(volume / MINI_VOLUME));
		DrawSmallFontPrintf(3, 264, bg, bg, "MP3NAME = %s", list_name);
